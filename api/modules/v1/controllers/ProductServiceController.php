...
class ProductServiceController extends Controller
{
	...
	*/	
	/*	$model = ProductService::find()
            ->where(['product_service_id' => $product_service_id])->one();
  */  //        ->orWhere(['user_id' => Yii::$app->user->id])->one();
	...
	*/
    ...
    public function actionProductServiceIdAndStep($product_service_id, $step)
    {
        $model = ProductService::findAll(['product_service_id'=>$product_service_id,''=>$step]);
        if (!$model)
        return (new ApiResponse)->error(null, ApiResponse::NO_CONTENT);
